export const translations = {
  es: {
    nav: {
      projects: "Proyectos",
      skills: "Habilidades",
      contact: "Contacto",
    },
sidebar: {
      role: "Desarrollador Web FULLSTACK",
      greeting: "¡Hola! Soy",
      firstName: "Juancho",
      degree: "Máster en Desarrollo Web Fullstack",
      bio: "Me apasiona el desarrollo web y disfruto diseñando toda la parte del FrontEnd, creando páginas visualmente elegantes y con todas las funciones necesarias para entregar un producto final totalmente funcional.",
      location: "Madrid, España",
      contactBtn: "Contactar Conmigo",
    },
    projects: {
      title: "Galería de Proyectos",
      countBadge: "Proyectos Destacados",
      demoBtn: "Live Demo",
      codeBtn: "GitHub",
    },
    skills: {
      title: "Habilidades & Tecnologías",
    },
    contact: {
      title: "Contacto",
      desc: "¿Tienes un proyecto en mente o deseas que trabajemos juntos? ¡No dudes en escribirme! Estoy disponible para nuevas oportunidades y colaboraciones.",
      nameLabel: "Nombre",
      namePlaceholder: "Tu nombre",
      emailLabel: "Email",
      emailPlaceholder: "tu@email.com",
      subjectLabel: "Asunto",
      subjectPlaceholder: "Asunto del mensaje",
      messageLabel: "Mensaje",
      messagePlaceholder: "Cuéntame sobre tu proyecto...",
      sendBtn: "Enviar Mensaje",
      sending: "Enviando...",
      successMsg: "¡Mensaje enviado! Te responderé pronto.",
      errorMsg: "Error al enviar. Intenta de nuevo o escribe directo al email.",
      errors: { name: "Nombre requerido", email: "Email inválido", subject: "Asunto requerido", message: "Mensaje requerido" },
      emailBtn: "Enviar Correo Directo",
      githubBtn: "Ver GitHub Profile",
    }
  },
  en: {
    nav: {
      projects: "Projects",
      skills: "Skills",
      contact: "Contact",
    },
    sidebar: {
      role: "Fullstack Web Developer",
      greeting: "Hello! I'm",
      firstName: "Juancho",
      degree: "Fullstack Web Development Master",
      bio: "I am passionate about web development and enjoy designing the entire FrontEnd, creating visually elegant pages with all the necessary features to deliver a fully functional final product.",
      location: "Madrid, Spain",
      contactBtn: "Contact Me",
    },
    projects: {
      title: "Projects Gallery",
      countBadge: "Featured Projects",
      demoBtn: "Live Demo",
      codeBtn: "GitHub",
    },
    skills: {
      title: "Skills & Technologies",
    },
    contact: {
      title: "Contact",
      desc: "Have a project in mind or want to work together? Feel free to reach out! I am available for new opportunities and collaborations.",
      nameLabel: "Name",
      namePlaceholder: "Your name",
      emailLabel: "Email",
      emailPlaceholder: "your@email.com",
      subjectLabel: "Subject",
      subjectPlaceholder: "Message subject",
      messageLabel: "Message",
      messagePlaceholder: "Tell me about your project...",
      sendBtn: "Send Message",
      sending: "Sending...",
      successMsg: "Message sent! I'll get back to you soon.",
      errorMsg: "Failed to send. Try again or email directly.",
      errors: { name: "Name required", email: "Invalid email", subject: "Subject required", message: "Message required" },
      emailBtn: "Send Direct Email",
      githubBtn: "View GitHub Profile",
    }
  }
};